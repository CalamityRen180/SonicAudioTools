...
namespace CsbBuilder.Builder
{
    public static class CsbBuilder
    {
        public static void Build(CsbProject project, string outputFileName)
        {
            ...
            HashSet<string> versionInfoSkipFields = new HashSet<string>();
            if (!project.HasExtensionSizeField)
            {
                versionInfoSkipFields.Add("ExtSize");
            }

            cueSheetTables.Add(new SerializationCueSheetTable
            {
                TableData = CriTableSerializer.Serialize(new List<SerializationVersionInfoTable>() { versionInfoTable }, CriTableWriterSettings.AdxSettings, versionInfoSkipFields),
                Name = "INFO",
                TableType = 7,
            });
            ...
            HashSet<string> cueSkipFields = new HashSet<string>();
            if (!project.CueHasFlag)
            {
                cueSkipFields.Add("flags");
            }

            cueSheetTables.Add(new SerializationCueSheetTable
            {
                TableData = CriTableSerializer.Serialize(cueTables, CriTableWriterSettings.AdxSettings, cueSkipFields),
                Name = "CUE",
                TableType = 1,
            });
            ...
            foreach (BuilderSoundElementNode soundElementNode in project.SoundElementNodes)
            {
                ...
                {
                    if (MainForm.Settings.EnableCPKCreation == false)
                    {

                        byte[] aaxdata = aaxArchive.Save();

                        string filepathnormalization = soundElementNode.Name.Replace('/', Path.DirectorySeparatorChar);

                        string directorydetector = Path.GetDirectoryName(soundElementNode.Name);


                        if (!string.IsNullOrEmpty(directorydetector))
                        {
                            if (!Directory.Exists($@"{project.BinaryDirectory}\{filepathnormalization}"))
                            {
                                Directory.CreateDirectory($@"{project.BinaryDirectory}\{directorydetector}");
                            }

                            File.WriteAllBytes($@"{project.BinaryDirectory}\{filepathnormalization}", aaxdata);
                        }
                        else
                        {
                            File.WriteAllBytes($@"{project.BinaryDirectory}\{filepathnormalization}", aaxdata);
            ...
            HashSet<string> soundElementSkipFields = new HashSet<string>();
            if (!project.HasSampleCountField)
            {
                soundElementSkipFields.Add("nsmpl");
            }

            cueSheetTables.Add(new SerializationCueSheetTable
            {
                TableData = CriTableSerializer.Serialize(soundElementTables, CriTableWriterSettings.AdxSettings, soundElementSkipFields),
                Name = "SOUND_ELEMENT",
                TableType = 4,
            });
