...
namespace CsbBuilder.Builder
{
    public static class CsbBuilder
    {
        public static void Build(CsbProject project, string outputFileName)
        {
            ...
            // TBLCUE requires 2-byte string pool alignment after
            // the first cue's strings only.
            CriTableWriterSettings cueSettings = CriTableWriterSettings.AdxSettings;
            cueSettings.AlignStringPoolAfterFirstRow = true;

            cueSheetTables.Add(new SerializationCueSheetTable
            {
                TableData = CriTableSerializer.Serialize(cueTables, cueSettings),
                Name = "CUE",
                TableType = 1,
            });
            ...
            if (project.VoiceLimitGroupNodes.Count > 0)
            {
